...
fn setup(opts: &mut Opts) {
    // Buffer options:
    // Highlights the current line (not currently working).
    opts.highlight_current_line = false;
    // Wrap lines instead of letting them go offscreen if too long.
    opts.wrap_lines = false;
    // Wether to wrap on words (not currently working).
    ...
    // How to show indentation levels.
    opts.indent_str = Some("▎");
    // Wether to show the indent_str on empty lines.
    opts.indent_str_on_empty = true;
    // Wether to show the indent_str on empty lines.
    opts.indent_tab_str = None;
    ...
    opts.enabled_hooks.default_opts_parser = true;
    // Automatically reloads the configuration when saving.
    opts.enabled_hooks.reload_on_save = true;
    // Automatically reloads Bufferss when external changes happen.
    opts.enabled_hooks.auto_reload_buffers = true;
    // Automatically shows the WhichKey widget.
    opts.enabled_hooks.show_whichkey = true;
    // Adds the default buffer widgets to every Buffer.
    opts.enabled_hooks.default_buffer_widgets = true;
    // Adds the default FooterWidgets to each window.
    opts.enabled_hooks.default_footer_widgets = true;
    // Save cursor positions when closing Bufferss.
    opts.enabled_hooks.cache_cursor_position = true;
}
```
