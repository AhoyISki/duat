...
impl<U: Ui> mode::Mode<U> for Regular {
    ...
    fn send_key(&mut self, pa: &mut Pass, key: KeyEvent, handle: Handle<Self::Widget, U>) {
        match key {
            ...
            // General
            key!(Char('P'), Mod::CONTROL) | key!(F1) => mode::set::<U>(RunCommands::new()),
            key!(Char('p'), Mod::CONTROL) => mode::set::<U>(RunCommands::new_with("edit ")),
            key!(Char('N'), Mod::CONTROL) => mode::set::<U>(RunCommands::new_with("open ")),
            key!(Char(','), Mod::CONTROL) => cmd::run("open ")
