...
//        if let Some((index, &range)) = self.0.iter().enumerate().find(|(_, r)| r.end >
// poke.start) {            // The first range after having its end removed by the poke.
//            let first_cut_range = TagRange { start: range.start, end: poke.start };
//
//            let mut ranges_iter = self.0.iter().enumerate().skip(index);
//
//            // Look for a range that can fit the end of the poke, and remove all ranges in
// between.            while let Some((last_checked, &range)) = ranges_iter.next() {
...
//                            }
//                            if let Some(form) = form_match.form {
//                                tags.insert((range.end.byte, Tag::PopForm(form)));
//                            }
//                        } else {
...
//                    let pokes = form_match.recur_match(lines, &mut ranges, info,
// pattern_slice).0;
//
...
//    // pattern bounds in it, then you don't need to update the syntax highlighting of other
// lines,    // (at least, not for that reason).
...
//    ) -> Vec<(LineInfo, usize)> {
//        let (mut info, last_match) = self.default.color_text(lines, range,
// self.last_match.clone());        self.last_match = last_match;
//
